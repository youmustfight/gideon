...
def build_index(index_save_path):
    print(f"INFO (gideon_faiss.build_index): Building... {index_save_path}")
    built_index = None
    # IF INDEX SAVED
    ...
    # IF NOT, BUILD
        ...
        if index_save_path == index_clip_save_path:
            ...
            print("INFO (gideon_faiss.build_index): shape =", image_index_embedding_sample.shape)
            # --- build index
            ...
            print("INFO (gideon_faiss.build_index): dimensions =", dimensions)
        ...
        if index_save_path == index_documents_save_path or index_save_path == index_sentences_save_path:
            ...
            print("INFO (gideon_faiss.build_index): shape =", text_index_embedding_sample.shape)
            # --- build index
    ...
    print("INFO (gideon_faiss.build_index): is_trained", built_index.is_trained)
    print("INFO (gideon_faiss.build_index): ntotal", built_index.ntotal)
    return built_index
# --- save
def save_index(index, index_save_path):
    print(f"INFO (gideon_faiss.save_index): saving '{index_save_path}'")
    faiss.write_index(index, index_save_path)
def save_indexes():
    print("INFO (gideon_faiss.save_indexes): saving")
...
def index_clip_add_image(image_filepaths, filename):
    print(f"INFO (gideon_faiss.clip_add_image): '{filename}'")
    image_embedding = index_clip_model.encode(image_filepaths) # returns numpy array w/ shape of (x inserted, params)
    print(f"INFO (gideon_faiss.clip_add_image): shape", image_embedding.shape)
    ...
    print(f"INFO (gideon_faiss.clip_add_image): ids", ids)
    index_clip.add_with_ids(image_embedding, ids)
    # Return ID?
    print(f"INFO (gideon_faiss.clip_add_image): index.ntotal", index_clip.ntotal)
    print(f"INFO (gideon_faiss.clip_add_image): index", index_clip)
    # OPTIMIZE: re-save index
    save_index(index_clip, index_clip_save_path)

def index_clip_search_images_by_text(text_query):
    print("INFO (gideon_faiss:clip_search_images_by_text): query", text_query)
    ...
    print("INFO (gideon_faiss:clip_search_images_by_text): text_embedding", text_embedding.shape)
    num_returned_results = 5 # what should this be?
    probabilities, ids = index_clip.search(text_embedding, num_returned_results)
    probabilities = probabilities[0]
    ids = ids[0]
    print("INFO (gideon_faiss:clip_search_images_by_text): probabilities", probabilities)
    print("INFO (gideon_faiss:clip_search_images_by_text): ids", ids)
...
async def index_documents_add_text(session, text, document_id=None, document_content_id=None):
    ...
    # OPTIMIZE: re-save index
    print("INFO (gideon_faiss:index_documents_add_text): save index")
    save_index(index_documents, index_documents_save_path)
...
async def index_sentences_add_text(session, text, document_id=None, document_content_id=None):
    ...
    # OPTIMIZE: re-save index
    print("INFO (gideon_faiss:index_sentences_add_text): save index")
    save_index(index_sentences, index_sentences_save_path)
