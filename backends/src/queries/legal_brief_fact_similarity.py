...
async def legal_brief_fact_similarity(session, case_id, organization_id, query_text=None):
    ...
    aigent_fact_embedder = await create_ai_action_agent(session, action=AI_ACTIONS.legal_brief_facts_similarity_embed, case_id=case_id, organization_id=organization_id)
    ...
    # 1. SEARCH
    # --- similarity search
    aigent_fact_similarity = await create_ai_action_agent(session, action=AI_ACTIONS.legal_brief_facts_similarity_search, case_id=case_id, organization_id=organization_id)
