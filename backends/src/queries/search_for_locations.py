...
from gideon_utils import get_documents_json, get_file_path, open_txt_file, similarity
import openai
...
# SEARCH QUERY
def search_for_locations_by_text_vector(text_query_vector):
  print('INFO (search_for_locations_by_vector.py): query all documents for vector', text_query_vector)
  # SETUP
  # --- files
  ...
  for json_document_payload in json_documents:
    ...
    for sentence_text_vector in json_document_payload['document_text_vectors_by_sentence']:
      ...
      if len(sentence_text_vector['text']) > 50:
        ...
        document_score = similarity(text_query_vector, sentence_text_vector['vector'])
        # --- push to locations w/ 
        location_to_push = {
...
if __name__ == '__main__':
    ...
    search_for_locations_by_text_vector(query_vector)