...
async def _index_document_audio_process_content(session, document_id: int) -> None:
    print("INFO (index_document_audio.py:_index_document_audio_process_content) started", document_id)
    ...
    if len(document_content) == 0:
        ...
        print(f"INFO (index_document_audio.py:_index_document_audio_process_content): Inserted new document content records")
    else:
        print(f"INFO (index_document_audio.py:_index_document_audio_process_content): already processed content for document #{document_id} (content count: {len(document_content)})")

    # 3. SAVE
    document.status_processing_content = "completed"
    session.add(document)
    print("INFO (index_document_audio.py:_index_document_audio_process_content) done", document_id)

async def _index_document_audio_process_embeddings(session, document_id: int) -> None:
    print('INFO (index_document_audio.py:_index_document_audio_process_embeddings): start')
    # SETUP
    document_query = await session.execute(sa.select(Document).where(Document.id == document_id))
    document = document_query.scalars().one()
    document_content_query = await session.execute(sa.select(DocumentContent).where(DocumentContent.document_id == document_id))
    document_content = document_content_query.scalars().all()
    print('INFO (index_document_audio.py:_index_document_audio_process_embeddings): document content', document_content)
    ...
    print('INFO (index_document_audio.py:_index_document_audio_process_embeddings): encoding sentences...', document_content_sentences)
    ...
    print('INFO (index_document_audio.py:_index_document_audio_process_embeddings): encoding sentences in chunks of 20...', document_content_sentences_20)
...
# INDEX AUDIO
async def index_document_audio(session, document_id) -> int:
    print(f"INFO (index_document_audio.py): indexing document #{document_id}")
    try:
        # PROCESS FILE & EMBEDDINGS
        print(f"INFO (index_document_audio.py): processing document #{document_id} content")
        await _index_document_audio_process_content(session=session, document_id=document_id)
        ...
        print(f"ERROR (index_document_audio.py):", err)
