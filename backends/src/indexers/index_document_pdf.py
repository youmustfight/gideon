...
async def _index_document_pdf_process_extractions(session, document_id: int) -> None:
    print('INFO (index_document_pdf.py:_index_document_pdf_process_extractions): start')
    document_query = await session.execute(sa.select(Document).where(Document.id == document_id))
    document = document_query.scalars().one()
    ...
    print(f'INFO (index_document_pdf.py:_index_document_pdf_process_extractions): len(document_text) = {len(document_content_text)}')
    # EXTRACT
    ...
    session.add(document)
    print('INFO (index_document_pdf.py:_index_document_pdf_process_extractions): done')
...
async def index_document_pdf(session, document_id) -> int:
    ...
    try:
        ...
        await _index_document_pdf_process_extractions(session=session, document_id=document_id)
