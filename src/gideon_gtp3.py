...
def gpt3_completion(prompt, engine='text-davinci-002', temp=0.4, top_p=1.0, max_tokens=2000, freq_pen=0.25, pres_pen=0.0, stop=['<<END>>']):
    max_retry = 2
    retry = 0
    while True:
        try:
            print('INFO (GPT3): gpt3_completion')
...
def gpt3_completion_repeated(prompt_file, text_to_repeatedly_complete, text_chunk_size=6000, return_list=False, dedupe=False):
    print('INFO (GPT3): gpt3_completion_repeated')
